...
static int createTopK(RedisModuleCtx *ctx, RedisModuleString **argv, int argc, TopK **topk) {
    ...
    if (argc == 6) {
        if ((RedisModule_StringToLongLong(argv[3], &width) != REDISMODULE_OK) || width < 1 || width > UINT32_MAX) {
            INNER_ERROR("TopK: invalid width");
        }
        if ((RedisModule_StringToLongLong(argv[4], &depth) != REDISMODULE_OK) || depth < 1 || depth > UINT32_MAX) {
