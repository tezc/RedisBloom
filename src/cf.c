...
#define CUCKOO_TRYCALLOC(...) RedisModule_TryCalloc ? RedisModule_TryCalloc(__VA_ARGS__) : RedisModule_Calloc(__VA_ARGS__)
...
CuckooFilter *CFHeader_Load(const CFHeader *header) {
    ...
    for (size_t ii = 0; ii < filter->numFilters; ++ii) {
        ...
        cur->data = CUCKOO_TRYCALLOC((size_t)cur->numBuckets * filter->bucketSize, sizeof(CuckooBucket));
        if (!cur->data) {
            goto error;
        }
    }
